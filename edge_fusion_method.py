...
class Edge_Fusion_Method():
    ...
    def up_samp(self, tensor1_need_up, tensor2):
        ...
        tensor2 = self.conv(tensor2, is1x1=True, filters=self.split_filters)
        return tensor2
    ...
    def pre_procession(self, input_tensor1, input_tensor2, input_tensor3):
        input_tensor1 = self.conv(input_tensor1, is1x1=True)
        input_tensor2 = self.conv(input_tensor2, is1x1=True)
        input_tensor3 = self.conv(input_tensor3, is1x1=True)
        for i in range(2):
            input_tensor1 = self.conv(input_tensor1, filters=120)
            input_tensor2 = self.conv(input_tensor2, filters=120)
            input_tensor3 = self.conv(input_tensor3, filters=120)
    ...
    def add_weight_edge(self, conv_height, conv_middle, conv_low):
        ...
        return conv_height, conv_middle, conv_low
    ...
    def call(self, input_tensor1, input_tensor2, input_tensor3):
        ...
        conv_height, conv_middle, conv_low = self.add_weight_edge(conv_height, conv_middle, conv_low)
        out_tensor = Concatenate()([conv_low, conv_middle, conv_height])
        out_tensor = self.conv(out_tensor, filters=64)
        return out_tensor
...
if __name__ == '__main__':
    ...
    out = edge_fusion_.call(input1, input2, input3)
