...
class ResNetFamily():
    ...
    def bn_conv_a(self, input_tensor, f_size, k_size, name, dila_rate=1):
        ...
        x = leak_relu(x, name='{}_AC'.format(name))
        return x
    ...
    def res_block1(self, input_tensor, f_size, name):
        ...
        out_tensor = leak_relu(out_tensor, name='{}_AC'.format(name))
        return out_tensor
    ...
    def res50(self, input_tensor, Flag=False):
        ...
        conv1 = self.bn_conv_a(conv1, f_size=f_size, k_size=3, name='conv1_3')
        # pool1 = MaxPool2D()(conv1)
        conv2 = Conv2D(f_size*2, 1, strides=2, padding='same', name='pool1')(conv1)
        for i in range(3):
            conv2 = self.res_block2(conv2, f_size=f_size*2, name='conv2_{}'.format(i))

        conv3 = Conv2D(f_size*4, 1, strides=2, padding='same', name='pool2')(conv2)
        for i in range(4):
            conv3 = self.res_block2(conv3, f_size=4 * f_size, name='conv3_{}'.format(i))

        conv4 = Conv2D(f_size * 8, 1, strides=2, padding='same', name='pool3')(conv3)
        for i in range(6):
            conv4 = self.res_block2(conv4, f_size=8 * f_size, name='conv4_{}'.format(i))


        if Flag:
            conv5 = Conv2D(f_size * 16, 1, strides=1, padding='same', name='pool4')(conv4)
            for i in range(3):
                conv5 = self.res_block2(conv5, f_size=16 * f_size, name='conv5_{}'.format(i))

        else:
            conv5 = Conv2D(f_size * 16, 1, strides=2, padding='same', name='pool4')(conv4)
            for i in range(3):
                conv5 = self.res_block2(conv5, f_size=16 * f_size, name='conv5_{}'.format(i))
        return conv1, conv2, conv3, conv4, conv5
